...
from src.tools.yfinance_api import (
    YFINANCE_AVAILABLE,
    get_prices_yfinance,
    get_financial_metrics_yfinance,
    search_line_items_yfinance,
    get_insider_trades_yfinance,
    get_company_news_yfinance,
    get_market_cap_yfinance,
)
...
def get_prices(ticker: str, start_date: str, end_date: str, api_key: str = None, data_provider: str = "yfinance") -> list[Price]:
    """Fetch price data from cache or API. Default uses Yahoo Finance (free, no API key required)."""
...
def get_financial_metrics(
    ...
    """Fetch financial metrics from cache or API. Default uses Yahoo Finance (free, no API key required)."""
    # Create a cache key that includes all parameters to ensure exact matches
...
def search_line_items(
    ...
    """Fetch line items from cache or API. Default uses Yahoo Finance (free, no API key required)."""
    # Route to appropriate data provider
    if data_provider == "yfinance":
...
def get_insider_trades(
    ...
    """Fetch insider trades from cache or API. Default uses Yahoo Finance (free, no API key required)."""
    # Create a cache key that includes all parameters to ensure exact matches
...
def get_company_news(
    ...
    """Fetch company news from cache or API. Default uses Yahoo Finance (free, no API key required)."""
    # Create a cache key that includes all parameters to ensure exact matches
    cache_key = f"{ticker}_{start_date or 'none'}_{end_date}_{limit}_{data_provider}"
...
def get_market_cap(
    ...
    """Fetch market cap from the API. Default uses Yahoo Finance (free, no API key required)."""
    # Route to appropriate data provider
    if data_provider == "yfinance":
